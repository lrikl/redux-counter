...
const localStorageMiddleware = store => next => action => {
  ...
  if (action.type.startsWith('counterSimple/') || action.type.startsWith('counterEmojis/')) { // Після того, як стан оновився, перевіряємо, чи була ця дія для конкретного лічильника по його name
